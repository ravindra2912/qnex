...
class OrderController extends Controller
{
    public function index(Request $request)
    {
		...
		//filters 
		if($request['search'] != null){ $orderLists = $orderLists->where('orders.id','LIKE','%'.$request['search'].'%'); }
		if($request['start_date'] != null){ $orderLists = $orderLists->where('orders.created_at','>=',$request['start_date']); }
		if($request['end_date'] != null){ $orderLists = $orderLists->where('orders.created_at','<=',date('Y-m-d', strtotime('+1 day', strtotime($request['end_date'])))); }
		if($request['status'] != null){ $orderLists = $orderLists->where('orders.status',$request['status']); }
		if($request['payment_type'] != null){ $orderLists = $orderLists->where('orders.payment_type',$request['payment_type']); }
		if($request['payment_status'] != null){ $orderLists = $orderLists->where('orders.payment_status',$request['payment_status']); }
		...
		foreach($orderstaus as $val){
			$val->orders = count(
				Orders::where('orders.status', $val->id)
				->join('orders_items as oi','oi.order_id','orders.id','right')
				->join('products as p','p.id','oi.product_id','right')
