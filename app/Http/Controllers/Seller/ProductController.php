...
class ProductController extends Controller
{
    ...
    public function index(Request $request)
    {
		...
		//filters 
		if($request['search'] != null){ $productLists = $productLists->where('id','LIKE','%'.$request['search'].'%')->orwhere('name','LIKE','%'.$request['search'].'%'); }
		if($request['start_date'] != null){ $productLists = $productLists->where('created_at','>=',$request['start_date']); }
		if($request['end_date'] != null){ $productLists = $productLists->where('created_at','<=',date('Y-m-d', strtotime('+1 day', strtotime($request['end_date'])))); }
		if($request['status'] != null){ $productLists = $productLists->where('status',$request['status']); }
    ...
    public function store(Request $request)
    {
        ...
        if($validator->fails()){ // Validation fails
            ...
            if($check_product_name == 0){ // Same name product is not exist with same category, sub category and sub category2 so insert it
                ...
                if($request->is_variants == 0){ $product->quantity = $request->quantity; }
    ...
    public function edit($id)
    {
        ...
        if(isset($productData) && isset($productData->id)){	
            ...
            $subCategoryData = Category::where('parent_id', $productData->category_id)
                                            ->where('status', 'Active')
                                           
                                            ->orderBy('name','ASC')
                                            ->get();

            $subCategory2Data = Category::where('parent_id', $productData->sub_category_id)
                                            ->where('status', 'Active')
			...
			if($array != null){
				...
				foreach($res as $val){
					if($c == 1){
						...
						if($var_name != null){ $var_name .= ','.$vname; }else{ $var_name .= $vname; }
					}else{
						$var_name = '';
						$var_ids = implode(',', $val);
						foreach($val as $v){
	...
	public function product_review(Request $request, $id)
    {
		...
		if($request['start_date'] != null){ $ProductReview = $ProductReview->where('created_at','>=',$request['start_date']); }
		if($request['end_date'] != null){ $ProductReview = $ProductReview->where('created_at','<=',date('Y-m-d', strtotime('+1 day', strtotime($request['end_date'])))); }
