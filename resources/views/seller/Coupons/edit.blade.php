			...
			<div class="card">
				...
				<div class="card-body">
					<div class="row" >
						...
						<div class="form-group col-md-3 col-sm-6">
							<label class="required">Select Sub Category2</label>
							<select class="form-control" name="sub_category2" id="sub_category2"  required>
								<option value="">Please Select Sub Category2</option>
							</select>
						</div>
						<div class="form-group col-md-3 col-sm-6">
							<label class="required">Product</label>
							<select class="select2" multiple="multiple" name="product_id" id="products" style="width: 100%;">
								<option value="" disabled>Please Select Products</option>
							</select>
						</div>
      ...
      <div class="container-fluid">
        <div class="row">
          ...
          <div class="col-md-8 col-sm-10">
					...
					<div class="col-md-12 custom-radio-ml">
					  <label class="form-label head_label" >Types</label>
						<div class="form-check radio radio-primary">
							  <input class="form-check-input" id="1" type="radio"  onchange="check_type(this.value)"  name="type" value="1" @if( $Coupons->coupon_type == 1) checked @endif >
							  <label class="form-check-label" for="1">Percentage</span></label>
						 </div>
						 <div class="form-check radio radio-primary">
							  <input class="form-check-input" id="2" type="radio" onchange="check_type(this.value)" name="type" value="2" @if( $Coupons->coupon_type == 2) checked @endif >
							  <label class="form-check-label" for="2">Free amount</span></label>
						 </div>
						 <div class="form-check radio radio-primary">
							  <input class="form-check-input" id="3" type="radio"  onchange="check_type(this.value)"  name="type" value="3" @if( $Coupons->coupon_type == 3) checked @endif >
							  <label class="form-check-label" for="3">Free shipping</span></label>
						 </div>
						<span class="text-danger">@error('coupon_type') {{$message}} @enderror</span> 
					</div>
					</div>
				</div>
			</div>
			...
			<div class="card" id="for_amt_per" style=" @if( $Coupons->coupon_type == 3 ||  $Coupons->coupon_type == null) display:none; @endif ">
				<div class="card-body">
					<label class="form-label head_label" >Value</label>	
					<div class="row g-3">
						<div class="col-md-4" id="type_coupon_amount" style="@if( $Coupons->coupon_type == 1) display:none; @endif">
							<label class="form-label" >Amount</label>
							<input class="form-control" name="coupon_amount" value="{{ $Coupons->coupon_amount }}" id="coupon_amount" type="number" placeholder="Enter amount">
							<span class="text-danger">@error('coupon_amount') {{$message}} @enderror</span> 
						</div>
						<div class="col-md-4" id="type_coupon_percent" style="@if( $Coupons->coupon_type == 2) display:none; @endif">
							<label class="form-label" >Percentage</label>
							<input class="form-control" name="coupon_percent" value="{{ $Coupons->coupon_percent }}" max="100" min="0" id="coupon_percent"  type="number" placeholder="Enter Percentage" required >
			...
			<div class="card" >
				<div class="card-body">	
					...
					<div class="row g-3" >
						<div class="col-md-12 custom-radio-ml">
							<div class="form-check radio radio-primary">
								  <input class="form-check-input" id="min_1" type="radio" name="minimum_requrment_type" @if( $Coupons->minimum_requrment_type == 0 ||  $Coupons->minimum_requrment_type == '') checked @endif onchange="check_mini_req(this.value)" value="0" >
								  <label class="form-check-label" for="min_1">None</span></label>
								  
							 </div>
							 <div class="form-check radio radio-primary">
									<input class="form-check-input" id="min_2" type="radio" name="minimum_requrment_type" @if( $Coupons->minimum_requrment_type == 1) checked @endif onchange="check_mini_req(this.value)"  value="1" >
									<label class="form-check-label" for="min_2">Minimum purchase amount (₹)</span></label>
									<input type="number" class="form-control" name="minimum_requrment_amt" value="{{ $Coupons->minimum_requrment }}" id="minimum_requrment_amt" style="@if( $Coupons->minimum_requrment_type != 1) display:none; @endif" placeholder="Enter Amount" >
									<span class="text-danger">@error('minimum_requrment_amt') {{$message}} @enderror</span>
							 </div>
							 <div class="form-check radio radio-primary">
									<input class="form-check-input" id="min_3" type="radio" name="minimum_requrment_type" @if( $Coupons->minimum_requrment_type == 2) checked @endif onchange="check_mini_req(this.value)" value="2" >
									<label class="form-check-label" for="min_3">Minimum quantity of items</span></label>
									<input type="number" class="form-control" name="minimum_requrment_qty" value="{{ $Coupons->minimum_requrment }}" id="minimum_requrment_qty" style="@if( $Coupons->minimum_requrment_type != 2) display:none; @endif" placeholder="Enter Qauntity" >
					...
					<div class="row">
						<div class="col-md-12">
							<div class="form-check">
									<input class="form-check-input" id="chk-ani" name="is_coupon_limit" @if( $Coupons->is_coupon_limit == 1) checked @endif value="1" onchange="usage_limit_show(this.checked)" type="checkbox" > <label class="form-check-label" for="chk-ani" >Limit number of times this discount can be used in total</label>
									<input type="number" class="form-control" value="{{ $Coupons->coupon_limit }}" name="coupon_limit" id="usage_limit" style="@if( $Coupons->is_coupon_limit != 1) display:none; @endif margin: 13px 0px;" placeholder="Enter Limit" >
									<span class="text-danger">@error('coupon_limit') {{$message}} @enderror</span>
							</div>
							<div class="form-check">
								  <input class="form-check-input" id="chk-ani1" name="once_per_user" @if( $Coupons->once_per_user == 1) checked @endif value="1" type="checkbox">
			...
			<div class="card" >
				<div class="card-body">	
					...
					<div class="row g-3" style="color: black;">			  
						...
						<div class="col-sm-3">
							<input class="form-control" name="active_time" type="time" value="{{ $Coupons->active_time }}" >
					...
					<div class="row g-3" style="color: black;margin-top: 10px;">			  
						...
						<div class="col-sm-3">
							<input class="form-control" name="end_time" type="time" value="{{ $Coupons->end_time }}" >
							<span class="text-danger">@error('end_time') {{$message}} @enderror</span>
						</div>
						
						<label class="col-sm-2 col-form-label mt-3">Status</label>
					...
					<div class="row g-3" style="color: black;margin-top: 10px;">	
						<div class="col-md-3">
							<div class="form-check">
								  <input class="form-check-input" id="chk-ani2" name="show_in_list" value="1" type="checkbox" @if( $Coupons->show_in_list == 1) checked @endif>
...
<script>
  ...
  <script type="text/javascript">
	...
	$("#category").change(function(){
        ...
        $("body").find("#products").find('option').remove();
    ...
    $("#sub_category").change(function(){
	  ...
	  $("#sub_category2").change(function(){
		...
		var sub_category = $("#sub_category").val();
		 var sub_category2 = $(this).val();
		
		var fd = new FormData();
        fd.append("category", category?category:'');
        fd.append("sub_category", sub_category?sub_category:'');
        fd.append("sub_category2", sub_category2?sub_category2:'');
